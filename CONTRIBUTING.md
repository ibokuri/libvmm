...
#### Comments
Often times, you'll also want to include comments in a patch that may not be
pertinent enough for inclusion in the final git log. For example, you may note
that a particular patch fixes issues raised from a previous version of the
patch. To do this, use the `--annotate` option to open the patch in an editor
so that you may modify it:
